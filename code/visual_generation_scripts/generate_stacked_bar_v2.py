...
def create_stacked_bar():
    ...
    # Plot bars
    p1 = ax.barh(labels_rev, low_roi_rev, color='#D3D3D3', label='Low ROI Zones', height=0.6)
    p2 = ax.barh(labels_rev, high_roi_rev, left=low_roi_rev, color='#F37021', label='High ROI Zones', height=0.6)
    ...
    for rect in p1:
        ...
        if width > 0:
            ax.text(rect.get_x() + width/2, rect.get_y() + rect.get_height()/2, 
                    f'{int(width)}%', ha='center', va='center', color='#2D2D2D', fontweight='bold', fontsize=8)
    ...
    output_path = os.path.join(output_dir, 'OPEX_Reallocation_Bar.png')
